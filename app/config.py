...
def _validate_secret(name: str, value: str, min_length: int = 32) -> str:
    ...
    if value_lower in WEAK_DEFAULTS:
        raise RuntimeError(
...
class Config:
    ...
    SECRET_KEY: str = _require_validated_secret("SECRET_KEY", 48)
    ANALYTICS_SALT: str = _require_validated_secret("ANALYTICS_SALT", 32)
    ...
    SQLALCHEMY_DATABASE_URI: str = "sqlite:///instance/blog.db"
    ...
    # Upload limits
    MAX_UPLOAD_SIZE: int = int(os.getenv("MAX_UPLOAD_SIZE", "2097152"))  
    OBSIDIAN_VAULT_ROOT: str = os.getenv("OBSIDIAN_VAULT_ROOT", "~/Main-Notes-Sync")
    MARKDOWN_ASSET_URL_PREFIX: str = os.getenv("MARKDOWN_ASSET_URL_PREFIX", "/static/img/vault")
    AUTO_SYNC_CONTENT: bool = os.getenv("AUTO_SYNC_CONTENT", "1") == "1"
    CONTENT_SYNC_INTERVAL_SECONDS: int = int(os.getenv("CONTENT_SYNC_INTERVAL_SECONDS", "5"))

    @property
    def ALLOWED_EXTENSIONS(self) -> FrozenSet[str]:
        """Allowed file extensions for upload."""
        return frozenset({".md", ".markdown", ".png", ".jpg", ".jpeg", ".webp"})
